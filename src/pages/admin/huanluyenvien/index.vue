  ...
  <div class="huanluyenvien-container">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4">
      <div>
        ...
        <p class="text-secondary mb-0">Theo dõi chuyên môn, lịch giảng dạy và danh sách học viên của từng huấn luyện
          viên (PT).</p>
      </div>
      <button
        class="btn btn-primary rounded-pill px-4 py-2 mt-3 mt-md-0 d-flex align-items-center gap-2 shadow-sm btn-add-pt"
        ...
        <div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
          <div class="d-flex justify-content-between align-items-start">
            <div>
              ...
              <h3 class="fw-bold text-dark my-1">{{ totalTrainers }}</h3>
        ...
        <div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
          <div class="d-flex justify-content-between align-items-start">
            <div>
              ...
              <h3 class="fw-bold text-dark my-1">{{ todayClasses }}</h3>
        ...
        <div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
          <div class="d-flex justify-content-between align-items-start">
            <div>
              ...
              <h3 class="fw-bold text-dark my-1">{{ averageRating }}</h3>
        ...
        <div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
          <div class="d-flex justify-content-between align-items-start">
            <div>
              ...
              <h3 class="fw-bold text-dark my-1">{{ teachingNow }}</h3>
              <span class="badge bg-info-subtle text-info border border-info rounded-pill px-2 py-0.5"
                style="font-size: 0.7rem;">
                Giờ cao điểm
              </span>
            </div>
            <div
              class="bg-info-subtle text-info rounded-4 d-flex align-items-center justify-content-center icon-card-box">
      ...
      <div
        ...
        <div class="input-group search-bar-input" style="max-width: 400px;">
          ...
          <input type="text" class="form-control border-start-0 border-light-custom"
            placeholder="Tìm theo tên, SĐT hoặc chuyên môn..." v-model="searchQuery" />
        </div>
        ...
        <table class="table align-middle custom-table">
          ...
          <tbody>
            <tr v-for="trainer in filteredTrainers" :key="trainer.id" class="hover-row">
              ...
              <td>
                <span class="text-secondary fw-semibold" style="font-size: 0.85rem;">{{ trainer.students }} học
                  viên</span>
              </td>
              <td>
                <span class="badge bg-light text-dark border rounded-pill px-2.5 py-1" style="font-size: 0.75rem;">{{
                  trainer.classes }} lớp / tuần</span>
              </td>
              <td>
                <span class="badge rounded-pill px-2.5 py-1.5 fw-bold" :class="trainer.statusClass"
                  style="font-size: 0.7rem;">
                  {{ trainer.status }}
                </span>
              </td>
              <td class="text-end">
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu dropdown-menu-end shadow border-0">
                    <li><a class="dropdown-item py-2" href="#" @click.prevent="editTrainer(trainer)"><i
                          class="fa-solid fa-pen-to-square me-2 text-secondary"></i>Chỉnh sửa thông tin</a></li>
                    <li><a class="dropdown-item py-2" href="#" @click.prevent="assignClass(trainer)"><i
                          class="fa-solid fa-calendar-plus me-2 text-secondary"></i>Phân lịch dạy</a></li>
                    <li>
                      <hr class="dropdown-divider">
                    </li>
                    <li><a class="dropdown-item py-2 text-danger" href="#" @click.prevent="deleteTrainer(trainer.id)"><i
                          class="fa-solid fa-trash-can me-2"></i>Xóa huấn luyện viên</a></li>
                  </ul>
                </div>
              </td>
            </tr>
      ...
      <div class="card border-0 shadow rounded-4 p-4 modal-box bg-white" style="width: 500px; max-width: 90%;">
        ...
        <form @submit.prevent="submitAddTrainer">
          <div class="mb-3">
            ...
            <input type="text" class="form-control" required v-model="newTrainer.name"
              placeholder="Ví dụ: Dustin Myers" />
          </div>
          <div class="mb-3">
            <label class="form-label fw-semibold text-secondary" style="font-size: 0.85rem;">Số Điện Thoại</label>
            <input type="tel" class="form-control" required v-model="newTrainer.phone"
    ...
    async getTrainers() {
      try {
        ...
        const res = await axios.get(
          "http://192.168.1.228:8000/api/admin/trainner",
          {
            headers: {
        ...
        // Summary
        this.totalTrainers = data.summary.total_trainers;
        this.todayClasses = data.summary.today_classes;
        this.averageRating = data.summary.average_rating;
        this.teachingNow = data.summary.teaching_now;

        // Danh sách PT
        this.trainers = data.data.map(item => ({
          id: item.id,
          avatar: item.avatar,
          name: item.name,
          phone: item.phone,
          ...
          rating: item.rating,
          students: item.total_members,
          classes: item.classes_this_week,
          status: item.status,

          statusClass:
            item.status === "Đang dạy"
